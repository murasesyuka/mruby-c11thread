...
mrb_queue_context_free(mrb_state *mrb, void *p) {
  if (p) {
    ...
    mtx_destroy(&context->mutex);
    mtx_destroy(&context->queue_lock);
    free(p);
  }
}
...
mrb_thread_init(mrb_state* mrb, mrb_value self) {
  ...
  if (!mrb_nil_p(proc)) {
    ...
    thrd_create(&context->thread, &mrb_thread_func, (void*) context);
  }
  return self;
}
...
mrb_mutex_try_lock(mrb_state* mrb, mrb_value self) {
  mrb_mutex_context* context = DATA_PTR(self);
  if (mtx_trylock(&context->mutex) == 0) {
    context->locked = TRUE;
...
mrb_queue_init(mrb_state* mrb, mrb_value self) {
  ...
  mtx_init(&context->mutex, mtx_plain);
  mtx_init(&context->queue_lock, mtx_plain);
